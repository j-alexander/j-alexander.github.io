...
type Offsets =
  { Next : Offset option       // the next offset to be committed
    ...
    Processed : Offset list }  // offsets of processed messages newer than (>) any active message (e.g., still working on 4L, but 5L and 6L are processed)
...
[<CompilationRepresentation(CompilationRepresentationFlags.ModuleSuffix)>]
module Checkpoint =
  let query (timeout:TimeSpan) (producer:Producer) (consumer:Consumer) (topic:Topic) : Async<Checkpoint> =
    async {
      let! metadata = 
